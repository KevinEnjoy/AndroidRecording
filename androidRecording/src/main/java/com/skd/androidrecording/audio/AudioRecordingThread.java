...
public class AudioRecordingThread extends Thread {
    ...
    public void run() {
    	...
    	AudioRecord record = new AudioRecord(AudioSource.MIC, /*AudioSource.MIC ,VOICE_RECOGNITION*/
							    			 SAMPLING_RATE,
							    			 AudioFormat.CHANNEL_IN_MONO,
							    			 AudioFormat.ENCODING_PCM_16BIT,
							    			 bufferSize);
    ...
    private void proceed() {
		...
		double frequency = (SAMPLING_RATE * peak)/magnitude.length;
		double frequencyIndex = ((SAMPLING_RATE * index)/magnitude.length);
		if(frequency>3000)Log.i("proceed: ","frequency="+frequency);
		if(frequencyIndex>0)Log.d("proceed: ","frequencyIndex="+frequencyIndex);
